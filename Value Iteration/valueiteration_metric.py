...
class ValueIterationAgent:
    def __init__(self, env: MapEnv, gamma=GAMMA, threshold=THRESHOLD, epsilon=EPSILON):
        ...
        self.episodes_to_converge = 0   # here: number of sweeps/iterations
        self.train_wall_time_sec = 0.0  # total computational time
    ...
    def train(self):
        ...
        self.train_wall_time_sec = time.perf_counter() - start_time
        self.episodes_to_converge = iteration
        print(f"Value iteration converged in {iteration} sweeps (final delta={delta:.2e}).")
        print(f"Training time: {self.train_wall_time_sec:.4f} seconds")
    ...
    def test_agent(self, env, n_episodes):
        ...
        self.epsilon = 0.0  # evaluate greedily
        ...
        # Return metrics for the final summary
        return {
...
if __name__ == "__main__":
    ...
    # Train (captures time and sweeps)
    agent.train()

    # Test and collect metrics
    metrics = agent.test_agent(training_env, 100)

    # Final required display
    print("\n=== Summary ===")
    print(f"Optimal path length: {metrics['avg_length']:.1f}")           # avg steps per episode
    print(f"Final Episode Reward: {metrics['avg_reward']:.3f}")          # average reward
    print(f"Final Success Rate: {metrics['success_rate']:.1%}")          # success rate
    print(f"Episode to convergence: {agent.episodes_to_converge}")       # VI sweeps
    print(f"Total Computational Time: {agent.train_wall_time_sec:.4f}s") # wall-clock time
